...
class Sdm_pusat_model extends CI_Model
{
     ...
     */
    public function get_sdm_by_pusat($pusat_id)
    {
        try {
    ...
    /**
     * Method untuk mendapatkan SDM berdasarkan ID pusat
     * @param int $pusat_id - ID pusat yang ingin ditampilkan
     */
    public function get_sdm_by_pusat_id($pusat_id)
    {
        try {
            // Cek apakah tabel sdm, jabatan_sdm, dan pusat ada
            if (!$this->db->table_exists('sdm') || !$this->db->table_exists('jabatan_sdm') || !$this->db->table_exists('pusat')) {
            ...
            // Query untuk mendapatkan SDM berdasarkan ID pusat
            $this->db->select('
                s.id as sdm_id,
                s.nama,
                s.nip,
            ...
            // Filter berdasarkan level pusat dan ID pusat
            $this->db->where('js.level', 'pusat');
            ...
            if ($query === FALSE) {
                $error = $this->db->error();
                log_message('error', 'Query SDM by pusat ID gagal: ' . $error['message']);
                log_message('error', 'Last query: ' . $this->db->last_query());
                return array();
            }
            ...
            // Log untuk debugging
            log_message('info', 'Query SDM berhasil untuk pusat_id: ' . $pusat_id . ', Total: ' . count($result));

            return $result;
        } catch (Exception $e) {
            log_message('error', 'Error dalam get_sdm_by_pusat_id: ' . $e->getMessage());
            return array();
        }
    }

    /**
     * Method untuk mendapatkan semua daftar pusat
     */
    public function get_all_pusat()
    {
        try {
            if (!$this->db->table_exists('pusat')) {
                log_message('error', 'Tabel pusat tidak ditemukan');
                return array();
            }
            ...
            if ($query === FALSE) {
                $error = $this->db->error();
                log_message('error', 'Query get all pusat gagal: ' . $error['message']);
                return array();
            }

            return $query->result();
        } catch (Exception $e) {
            log_message('error', 'Error dalam get_all_pusat: ' . $e->getMessage());
            return array();
        }
    }

    /**
     * Method untuk mendapatkan info pusat berdasarkan slug atau nama
     */
    public function get_pusat_by_slug($slug)
    {
        try {
            if (!$this->db->table_exists('pusat')) {
                log_message('error', 'Tabel pusat tidak ditemukan');
            ...
            if ($query === FALSE) {
                $error = $this->db->error();
                log_message('error', 'Query get pusat by slug gagal: ' . $error['message']);
                return null;
            }

            return $query->row();
        } catch (Exception $e) {
            log_message('error', 'Error dalam get_pusat_by_slug: ' . $e->getMessage());
            return null;
        }
    }
}
