...
class Nav_model extends CI_Model
{
	...
	// Navigasi Pendidikan
	public function nav_pendidikan()
	{
		$this->db->select('*');
		$this->db->from('pages');
		$this->db->where(array(
			'jenis_pages'	=> 'Pendidikan',
			'status_pages'	=> 'Publish'
		));
		$this->db->order_by('urutan', 'ASC');
		$query = $this->db->get();
